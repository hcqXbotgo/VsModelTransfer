...
DEFAULT_QUANT = Path(
    '/home/dragonfly/anaconda3/envs/Falcon2VsSocQuantEnv/bin/StatlasQuant')
DEFAULT_PYTHON = Path(
    '/home/dragonfly/anaconda3/envs/Falcon2VsSocQuantEnv/bin/python')
...
def add_images(mode, kind, paths, dry_run):
    if not paths:
        raise SystemExit('{} requires one or more image paths'.format(kind))
    tool = require(MODES_ROOT / mode / 'tools' / 'dataset.py',
                   '{} dataset manager'.format(mode))
    python = require(executable('STATLAS_PYTHON', DEFAULT_PYTHON, 'python3'),
                     'Python')
    dataset_root = MODES_ROOT / mode / 'datasets'
    dataset_kind = 'eval' if kind == 'add-eval' else 'calibration'
    run_command([python, tool, 'add', '--root', dataset_root,
                 '--kind', dataset_kind] + [Path(item) for item in paths], dry_run)
...
def main():
    ...
    parser.add_argument(
        ...
        choices=('quant', 'eval', 'visualize', 'compile', 'validate',
                 'status', 'all', 'add-eval', 'add-calibration'))
    ...
    elif args.operation in ('add-eval', 'add-calibration'):
        add_images(args.mode, args.operation, args.paths, args.dry_run)
